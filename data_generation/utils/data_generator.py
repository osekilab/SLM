...
class Generator:
    ...
    def generate_paradigm(self, number_to_generate=1000, rel_output_path=None, absolute_path=None):
        ...
        while len(past_sentences) < number_to_generate:
            if len(past_sentences) % 10 == 0:
                print(f"{len(past_sentences)} < {number_to_generate}")
            try:
...
class BenchmarkGenerator(Generator):
    ...
    def __init__(self,
        ...
        self.data_fields = ["sentence_good", "sentence_bad", "sentence_past-ed", "sentence_base-ed", "sentence_copy", "sentence_rr", "sentence_irr", "sentence_bare", "one_prefix_prefix", "two_prefix_prefix_good", "two_prefix_prefix_bad", "verb_type"]
