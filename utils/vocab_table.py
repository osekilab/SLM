...
def get_all_by_partial_match(label, text, table=vocab):
    """
    :param label: string. field name.
    :param text: string. part of label value.
    :param table: ndarray of vocab items.
    :return: Table restricted to all entries where the substring "text" is contained in the field "label" value
    """
    return np.array(list(filter(lambda x: text in x[label], table)), dtype=table.dtype)
